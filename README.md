...
5. 单Excel多sheet导出支持(**2022/1/18 正在实现支持**)
...
3. Excel的Sheet名字限制
    - 不允许重名的Sheet名字
    - Sheet名blacklist开头表示不参与导表(**2022/1/18 正在实现支持**)
4. 字段名要求
    - 非注释类型字段必须配置字段名
    - 非注释类型配置的字段名不能为空
...
6. 分隔符配置限制
    ...
    - T[]类型数据必须指定分隔符 
7. 数据配置限制
    - 第一列必须为int类型，用于配置id(**2022/1/18 string作为id正在实现支持**)
    - 第一列id配置不能为空
    - 第一列id不能重复
...
第二行写注释如果有空格符号会导致该表格无法生成对应的数据结构代码

解决方案:

具体原因是*.xb文件的正则匹配上不允许注释中间出现任意空格符号，暂时没支持，所以临时解决方案是不要再注释中间写空格符号。
